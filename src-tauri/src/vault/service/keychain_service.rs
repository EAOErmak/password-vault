...
impl KeychainService {
    ...
    pub fn get_session() -> Result<Option<AutoUnlockPayload>, VaultError> {
        ...
        match entry.get_password() {
            Ok(json) => {
                let payload: AutoUnlockPayload = serde_json::from_str(&json)
                    .map_err(|e| VaultError::Internal(format!("Failed to deserialize payload: {}", e)))?;
                Ok(Some(payload))
            }
            Err(keyring::Error::NoEntry) => Ok(None),
            Err(e) => Err(VaultError::Internal(format!("Failed to retrieve from OS Keychain: {}", e))),
        }
    }
